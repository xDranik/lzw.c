...
#define DICT_SIZE         (1 << 9)
#define NODE_NULL         (-1)
#define CHARS_IN_DICT_ROW (7)
...
static code_t lzw_add_str(lzw_t *ctx, code_t code, char c)
{
   ...
   fileLocation = (code * CHARS_IN_DICT_ROW) + 2;
   fseek(dictFile, fileLocation, SEEK_SET);
   fread(newValue, 2, 1, dictFile);

   prev = code;
   tmp = (unsigned char*) (&prev);
   dictRow[0] = tmp[0];
   dictRow[1] = tmp[1];

   first = -1;
   tmp = (unsigned char*) (&first);
   dictRow[2] = tmp[0];
   dictRow[3] = tmp[1];

   dictRow[4] = newValue[0];
   dictRow[5] = newValue[1];

   dictRow[6] = c;

   fileLocation = ctx->max * CHARS_IN_DICT_ROW;
   fseek(dictFile, fileLocation, SEEK_SET);
   fwrite(dictRow, 7, 1, dictFile);

   fileLocation = (code * CHARS_IN_DICT_ROW) + 2;
   fseek(dictFile, fileLocation, SEEK_SET);
   
   tmp = (unsigned char*)(& (ctx->max) );
   newValue[0] = tmp[0];
   newValue[1] = tmp[1];
